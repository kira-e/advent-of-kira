...
const winningShapeMap = new Map<Shape, Shape>(
  Array.from(loosingShapeMap.entries()).map(
    ([shape1, shape2]: [Shape, Shape]) => [shape2, shape1]
  )
);
...
const points1 = games
  .map((shapes) => {
    ...
    const playerPoints =
      shapesPoints.get(playerShape) + gameResultPoints.get(winner);
